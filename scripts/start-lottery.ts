...
const main = async () => {
    ...
    if ( networkName === "testnet" || networkName === "mainnet" ) {
        ...
        try {
            ...
            // Get ticket price (denominated in $LuChow), for a given network.
            ...
            const tx = await contract.startLottery(
                ...
                config.Burn[networkName],
                config.Treasury[networkName],
                config.Charity[networkName],
                { from: operator.address, gasLimit: 500000, gasPrice: _gasPrice.mul(2) }
            );

            const message = `[${new Date().toISOString()}] network=${networkName} block=${_blockNumber} message='Started lottery' hash=${tx?.hash} signer=${operator.address}`;
            console.log(message);
            logger.info({ message });
        } catch (error) {
            const message = `[${new Date().toISOString()}] network=${networkName} message='${error}' signer=${
              operator.address
            }`;
